...
var exchangeGoogleCode = (nodeRuntime) => {
...
var onCronTrigger = (runtime2) => {
  runtime2.log("Zene identity workflow triggered");
  const googleSub = runtime2.runInNodeMode(exchangeGoogleCode, consensusIdenticalAggregation())().result();
  runtime2.log("Google sub obtained via confidential compute");
  const hashInput = `salt:demo||sub:${googleSub}||chain:${runtime2.config.chainId}||app:${runtime2.config.appId}`;
  runtime2.log(`Identity hash input: ${hashInput}`);
  runtime2.log("Identity hash computed — ready for on-chain registration");
  return {
